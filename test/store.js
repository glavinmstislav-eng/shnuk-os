// store.js — Магазин приложений

(function() {
    'use strict';

    console.log('[Store] Загрузка магазина...');
    ...
    // ============================================
    // КАТАЛОГ (без заметок и калькулятора)
    // ============================================
    const CATALOG_ITEMS = [
        {
            id: 'shnuk-site-store',
            name: 'Shnuk Company Site',
            description: 'Официальный сайт Shnuk Company',
            author: 'Shnuk Team',
            version: '1.0.0',
            icon: null,
        ...
        },
        {
            ...
            name: 'Shnuk Projects',
            description: 'Проекты Shnuk Company',
            author: 'Shnuk Team',
            version: '1.0.0',
            icon: null,
    ...
    function loadFile(filename) {
        return new Promise(function(resolve) {
            ...
            fetch(filename)
                ...
                .catch(function(err) {
                    console.error('[Store] Ошибка загрузки ' + filename + ':', err);
                    resolve('');
                });
        });
    }
    ...
    function loadCatalog() {
        return Promise.all(
            ...
            loadedCatalog = items;
            console.log('[Store] ✅ Каталог загружен:', items.length, 'приложений');
            return items;
        });
    }
    ...
    function installApp(appData) {
        try {
            ...
            const appRecord = {
                ...
                author: appData.author || 'Неизвестно',
            ...
            console.log('[Store] ✅ Установлено:', appData.name);
            ...
            console.error('[Store] Ошибка установки:', e);
            return false;
        }
    }
    ...
    function uninstallApp(appId) {
        try {
            ...
            console.error('[Store] Ошибка удаления:', e);
            return false;
        }
    }
    ...
    function createUI() {
        ...
        if (!document.getElementById('storeStyles')) {
            ...
            style.textContent = `
                ...
                .store-header button {
                    ...
                    font-family: 'ST-SimpleSquare', monospace; transition: all 0.2s;
                }
                ...
                .app-card:hover {
                    border-color: #cc0000; background: #fff;
                ...
                .app-card .app-desc { font-size: 13px; color: #888; margin-bottom: 8px; line-height: 1.4; }
                .app-card .app-meta { font-size: 11px; color: #aaa; }
                .app-card .app-actions { display: flex; flex-direction: column; gap: 8px; flex-shrink: 0; }
                ...
                .upload-zone .icon { font-size: 64px; display: block; margin-bottom: 16px; color: #ccc; }
                .upload-zone .title { font-size: 18px; font-weight: 600; color: #333; margin-bottom: 8px; }
                ...
                .html-input-zone .install-btn {
                    ...
                    font-weight: 600; transition: background 0.2s; margin-top: 12px;
                }
                ...
                .empty-message .icon { font-size: 64px; display: block; margin-bottom: 16px; }
                ...
                @keyframes storeSpin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }
        ...
        if (loadedCatalog.length === 0) {
            ...
            loading.innerHTML = '<div class="spinner"></div>Загрузка каталога...';
    ...
    function renderCatalog(container) {
        ...
        loadedCatalog.forEach(app => {
            ...
            const iconChar = app.isUrl ? '🌐' : app.name.charAt(0);

            card.innerHTML = `
                <div class="${iconClass}">${app.icon ? `<img src="${app.icon}" style="width:70%;height:70%;object-fit:contain;">` : iconChar}</div>
                <div class="app-info">
            ...
            card.querySelector('button').addEventListener('click', function() {
                if (isInstalled(app.id)) {
                    if (confirm('Удалить приложение "' + app.name + '"?')) {
                        uninstallApp(app.id);
                        renderTab('catalog');
                        if (window.L) L.notify('Приложение удалено', { type: 'info' });
                    }
                } else {
                    if (installApp(app)) {
                        renderTab('catalog');
                        if (window.L) L.notify('"' + app.name + '" установлено!', { type: 'success' });
                    } else {
                        if (window.L) L.notify('Ошибка установки', { type: 'error' });
                    }
                }
            });
    ...
    function renderInstall(container) {
        ...
        uploadZone.innerHTML = `
            ...
            <div class="title">Перетащите HTML файл сюда</div>
        ...
        inputZone.innerHTML = `
            ...
            <div class="field-row">
                <div class="field">
                    ...
                    <input type="text" id="htmlAppName" placeholder="Моё приложение" />
                </div>
                <div class="field">
                    <label>Автор</label>
                    <input type="text" id="htmlAppAuthor" placeholder="Неизвестно" />
                </div>
                <div class="field">
                    <label>Версия</label>
            ...
            <div class="field" style="margin-bottom:12px;">
                ...
                <input type="text" id="htmlAppDesc" placeholder="Краткое описание" />
            </div>
            <div>
                <label>HTML код</label>
                <textarea id="htmlCodeInput" placeholder="<!DOCTYPE html>
<html>
...
</html>"></textarea>
            </div>
            <button class="install-btn" id="htmlInstallBtn">📦 Установить приложение</button>
        `;
        ...
        function loadHtmlFileFromUser(file) {
            ...
            reader.onload = function(e) {
                ...
                if (window.L) L.notify('HTML загружен', { type: 'info' });
            };
        ...
        document.getElementById('htmlInstallBtn').addEventListener('click', function() {
            ...
            const author = document.getElementById('htmlAppAuthor').value.trim() || 'Неизвестно';
            const version = document.getElementById('htmlAppVersion').value.trim() || '1.0.0';
            const description = document.getElementById('htmlAppDesc').value.trim() || 'HTML приложение';

            if (!name) { alert('Введите название'); return; }
            if (!html) { alert('Вставьте HTML код'); return; }
            ...
            if (installApp(appData)) {
                if (window.L) L.notify('"' + name + '" установлено!', { type: 'success' });
    ...
    function renderInstalled(container) {
        ...
        installed.forEach(app => {
            ...
            card.innerHTML = `
                <div class="app-icon">${app.icon ? `<img src="${app.icon}" style="width:70%;height:70%;object-fit:contain;">` : app.name.charAt(0)}</div>
                <div class="app-info">
                    <div class="app-name">${app.name}</div>
                    <div class="app-desc">${app.description || 'HTML приложение'}</div>
                    <div class="app-meta">${app.author || 'Неизвестно'} • v${app.version || '1.0.0'}</div>
                </div>
                <div class="app-actions">
            ...
            card.querySelector('.delete-btn').addEventListener('click', function() {
                if (confirm('Удалить "' + app.name + '"?')) {
                    uninstallApp(app.id);
                    renderTab('installed');
                    if (window.L) L.notify('Удалено', { type: 'info' });
                }
            });
