...
(function() {
    ...
    function resetProSettings() {
        ...
        const bsV = document.getElementById('camBrightnessVal');
        const csV = document.getElementById('camContrastVal');
        const ssV = document.getElementById('camSaturationVal');

        if (bs) bs.value = '100';
        if (cs) cs.value = '100';
        if (ss) ss.value = '100';
        if (bsV) bsV.textContent = '100';
        if (csV) csV.textContent = '100';
        if (ssV) ssV.textContent = '100';

        document.querySelectorAll('.camera-zoom-btn').forEach(b => {
    ...
    function toggleProMode() {
        proMode = !proMode;
        const panel = document.getElementById('cameraProPanel');
        const btn = document.getElementById('cameraProToggleBtn');
        if (panel) panel.classList.toggle('visible', proMode);
        if (btn) btn.classList.toggle('active', proMode);
    }
    ...
    function createUI() {
        ...
        if (!document.getElementById('cameraStyles')) {
            ...
            style.textContent = `
                ...
                @media (max-width: 500px) {
                    .camera-header h1 { font-size: 14px; }
                    .camera-pro-toggle { font-size: 11px; padding: 5px 10px; }
                    .camera-row label { min-width: 70px; font-size: 9px; }
                    .camera-shutter { width: 54px; height: 54px; }
                    .camera-main-row { gap: 24px; }
                }
            `;
        ...
        header.innerHTML = `
            ...
            <div class="camera-header-actions">
                <button class="camera-pro-toggle" id="cameraProToggleBtn">ПРОФИ</button>
                <button class="camera-close-header" id="cameraCloseBtn">✕</button>
            </div>
        `;
        ...
        proPanel.className = 'camera-pro-panel';
        proPanel.id = 'cameraProPanel';
        proPanel.innerHTML = `
            <div class="camera-pro-header">
                <span class="camera-pro-header-title">Профессиональный режим</span>
                <button class="camera-pro-reset" id="cameraProReset">Сбросить</button>
            </div>
        ...
        controls.innerHTML = `
            <div class="camera-zoom-row" style="margin-bottom:12px;">
        ...
        document.getElementById('cameraProToggleBtn').addEventListener('click', toggleProMode);
        document.getElementById('cameraProReset').addEventListener('click', resetProSettings);
